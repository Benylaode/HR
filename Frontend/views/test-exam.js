...
const dummyCategories = {
    1: {
        ...
        questions: [
            ...
            {
                id: 3,
                question: 'Seorang ASN harus memiliki integritas tinggi dalam menjalankan tugasnya.',
                type: 'true_false',
                options: ['Benar', 'Salah'],
                correctAnswer: 0,
                points: 5
            ...
            {
                id: 4,
                question: 'UUD 1945 pertama kali disahkan oleh PPKI pada tanggal 18 Agustus 1945.',
                type: 'true_false',
                options: ['Benar', 'Salah'],
                correctAnswer: 0,
                points: 5
        ...
        questions: [
            {
                ...
                type: 'multiple_choice',
                options: ['Semua A adalah C', 'Semua C adalah A', 'Tidak ada A yang C', 'Sebagian A adalah C'],
                correctAnswer: 0,
                points: 10
            },
            {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!testState.token || !testState.candidateName) {
        alert('Link test tidak valid! Hubungi admin untuk mendapatkan link yang benar.');
        return;
    }

    // Parse token to get category ID
    const tokenParts = testState.token.split('_');
    testState.categoryId = parseInt(tokenParts[1]);
    ...
    document.getElementById('candidateName').textContent = `Kandidat: ${testState.candidateName}`;
});
...
function setupSecurity() {
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.hidden && testState.isActive) {
            ...
            showWarningOverlay('Anda terdeteksi keluar dari halaman test! Ini akan dihitung sebagai pelanggaran.');
        }
    });
    ...
    window.addEventListener('blur', () => {           
        if (testState.isActive) {
            ...
            showWarningOverlay('Anda terdeteksi berpindah ke aplikasi lain! Tetap fokus pada test.');
        }
    });
    ...
    document.addEventListener('fullscreenchange', () => {
        if (!document.fullscreenElement && testState.isActive) {
            ...
            showWarningOverlay('Anda keluar dari fullscreen mode! Test harus dilakukan dalam fullscreen.');
            enterFullscreen();
        }
    });
...
function showViolation(message) {
    ...
    if (testState.violations >= testState.maxViolations) {
        alert('Anda telah mencapai batas maksimal pelanggaran! Test akan disubmit otomatis.');
        submitTest(true);
    }
}
        ...
        <div class="question-card">
            <div class="mb-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <span class="text-gray-600 text-sm">
                        <i class="fas fa-star text-yellow-500"></i> ${question.points} poin
                    </span>
                </div>
            ...
            <div class="space-y-3">
                ${question.options.map((option, index) => `
                    <button
                        class="option-btn ${testState.answers[testState.currentQuestionIndex] === index ? 'selected' : ''}"
...
function updateProgress() {
    ...
    document.getElementById('progressText').textContent =
        `Soal ${testState.currentQuestionIndex + 1} dari ${testState.questions.length}`;
}

// Render navigation dots
function renderNavigationDots() {
    const container = document.getElementById('navigationDots');
    container.innerHTML = testState.questions.map((_, index) => {
        ...
        return `
            <button class="${classes.join(' ')}" onclick="goToQuestion(${index})">
                ${index + 1}
            </button>
        `;
    }).join('');
...
function submitTest(forced = false) {
    ...
    if (!forced && unanswered > 0) {
        if (!confirm(`Anda masih memiliki ${unanswered} soal yang belum dijawab. Yakin ingin submit?`)) {
            return;
        }
    }
...
function showResults(score, correct, wrong) {
    ...
    if (score >= 80) {
        ...
        resultMessage.textContent = 'Bagus! Anda telah menyelesaikan test dengan baik.';
    } else {
