...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 z-50 bg-white border-r border-[var(--secondary-100)]">
        <div className="flex flex-col h-full bg-white">
          <div className="p-6 flex items-center justify-center border-b border-[var(--secondary-50)]">
            <Image 
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 flex z-50">
          ...
          <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white shadow-2xl transform transition-transform animate-slide-right">
            <div className="flex items-center justify-between p-4 border-b border-[var(--secondary-100)]">
               ...
               <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="p-2 -mr-2 text-[var(--secondary-400)] hover:text-red-500 hover:bg-red-50 rounded-lg"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
